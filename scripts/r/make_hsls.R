...
hsl_dir <- file.path(dat_dir, "PGPH9", "ASCII")
...
col_sta <- c(1,342,344,366,531,533,535,537,751,757,2326,9356)
col_end <- c(5,343,345,371,532,534,536,538,752,758,2327,9362)
col_nms <- c("stuid","x1sex","x1race","x1stdob",
             "x1famincome","x1poverty","x1poverty130","x1poverty185",
             "x1region","x1state","x4fb16enrstat","p1income")

## read in
df <- vroom_fwf(file.path(hsl_dir, "f2student.dat"),
...
df <- df |>
  ...
  ## filter out missing female (only 1)
  filter(female != 2,
         raceeth != 7) |>
...
df_brr <- readRDS(file.path(cln_dir, "hsls_weights.RDS")) |>
  rename(stuid = stu_id) |>
